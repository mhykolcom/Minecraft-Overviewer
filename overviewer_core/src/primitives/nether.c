    ...
    for (x = -1; x < WIDTH + 1; x++) {
        for (z = -1; z < DEPTH + 1; z++) {
            ...
            for (y = NETHER_ROOF - 1; y >= 0; y--) {
                ...
                if (block_class_is_subset(blockid, (mc_block_t[]){block_bedrock, block_netherrack, block_quartz_ore, block_lava, block_nether_gold_ore, block_ancient_debris, block_soul_soil, block_soul_sand, block_blackstone, block_basalt}, 10))
